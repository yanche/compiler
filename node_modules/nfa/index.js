...
var printEpsilonClosure = function (states) {
    for(var i = 0 ; i <states.length; ++i) {
        var state = states[i];
        console.log(['epsilon closure of', state.getName(), ':', state.getEpsilonClosure().getStates().map(function(s){ return s.getName(); }).join(',')].join(' '));
    }
};
...
var test = function (states, str) {
    ...
    while(states.length > 0 && i < slen) {
        ...
        for(var j = 0; j < states.length; ++j) {
            ...
            var mstates = utility.genToArr(tset.values());
            for(var m = 0; m < mstates.length; ++m)
                movingstates.add(mstates[m]);
        }
        states = epsilonClosureOfStates(utility.genToArr(movingstates.values()));
    }
...
var epsilonClosureOfStates = function (states) {
    ...
    return utility.genToArr(eclosure.values());
};
