...
export class LexReturn extends StageReturn {
    ...
    constructor(accept: boolean, tokens: Array<l.Token>, errmsg?: string, errcode?: number) {
        super(accept, errmsg, errcode);
        if (accept) {
            if (tokens == null) throw new Error('must provide tokens of parse tree if accepted');
        }
        else {
            if (tokens != null) throw new Error('tokens must not be specified if not accepted');
        }
        this._tokens = tokens;
    }
