...
function bulkValidate(arr, nfa) {
    for (let item of arr) {
        assert.equal(item.expected, nfa.accept(item.str.split('')));
    }
}
;
function acceptStrings(arr, nfa) {
...
describe('nfa-acceptance', function () {
    describe('no epsilon move', function () {
        it('1 state', function () {
            ...
            ], index_1.createNFA([tran(0, 0, 'a')], [0], [0]));
        });
        it('2 states', function () {
            bulkValidate([
                { str: 'aaaaa', expected: false },
                { str: 'a', expected: true }
            ], index_1.createNFA([tran(0, 1, 'a')], [0], [1]));
        });
        it('3 states', function () {
            bulkValidate([
                { str: 'ab', expected: true },
                { str: 'a', expected: false }
            ], index_1.createNFA([tran(0, 1, 'a'), tran(1, 2, 'b')], [0], [2]));
        });
    });
