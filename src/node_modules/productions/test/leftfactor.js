...
function validate(pset, expected) {
    ...
    for (let i = 0; i < expected.length; ++i) {
        ...
        assert.equal(true, utility.arrayEquivalent(item.rhsArr, pset.getProds(pset.getSymNum(item.lhs)).map(p => pset.getProdRef(p).rnums), function (test, real) {
            ...
            for (let q = 0; q < test.length; ++q) {
                ...
                if (pset.getSymNum(titem.str) !== symnum || titem.terminal !== pset.isSymNumTerminal(symnum))
                    return false;
            }
            return true;
        }));
    ...
    it('simple 3: 2 left factor for one non-terminal', function () {
        ...
        for (let i = 0; i < 3; ++i) {
            ...
            if (rhs.length === 2) {
                ...
                if (rhs[0] === lfprodset.getSymNum('T'))
                    new1 = r1sym;
                else
                    new2 = r1sym;
            }
...
//# sourceMappingURL=leftfactor.js.map