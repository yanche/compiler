...
export class Production {
    ...
    print(): this { console.log(this._literal); return this; }
...
export class ProdSet {
    private _startnontnum: number;
    private _prodliteralset: Set<string>;
    private _nontprodmap: Map<number, Array<number>>;
    private _numprodmap: Map<number, ProductionRef>;
    private _symnummap: utility.BidirectMap<string>;
    private _termict: number;
    private _totalsymct: number;
    private _nullableNonTerminals: Set<number>;
    private _firstSet: Map<number, Set<number>>;
    private _followSet: Map<number, Set<number>>;
    ...
    constructor(prods: Iterable<Production>) {
        ...
        let numprodmap = new Map<number, ProductionRef>(), nontprodmap = new Map<number, Array<number>>() //map from number of non-terminal to number its productions;
        let prodliteralset = new Set<string>(), startnontnum: number, reachedges: Array<utility.Edge> = [], prodidgen = new utility.IdGen();
        ...
        for (let prod of prods) {
            ...
            if (prodliteralset.has(prodliteral)) throw new Error('duplicate production is not allowed: ' + prodliteral);
    ...
    getProdRef(prodnum: number): ProductionRef { return this._numprodmap.get(prodnum); }
    getProdSize(): number { return this._numprodmap.size; }
    //getAllProds(): Array<number> { return _.range(this.getProdSize()); }
    print(): this { for (let prodliteral of this._prodliteralset) console.log(prodliteral); return this; }
    firstSet(): Map<number, Set<number>> {
        if (this._firstSet != null) return this._firstSet;
        let nullablenont = this.nullableNonTerminals(), retmap = new Map<number, Set<number>>();
        let edges: Array<utility.Edge> = [], nonterminals = this.getNonTerminals();
        for (let nont of nonterminals) {
            retmap.set(nont, new Set<number>());
        ...
        for (let t of this.getTerminals()) retmap.set(t, new Set<number>().add(t));
        this._firstSet = retmap;
        return retmap;
    }
    followSet(): Map<number, Set<number>> {
        if (this._followSet != null) return this._followSet;
        let retmap = new Map<number, Set<number>>(), firstSetMap = this.firstSet(), nullablenonterminals = this.nullableNonTerminals(), edges: Array<utility.Edge> = [];
        addToMapOfSet(retmap, this._startnontnum, 0); //0 here is the '$'
        for (let nont of this.getNonTerminals()) {
            if (!retmap.has(nont)) retmap.set(nont, new Set<number>());
            for (let prodid of this.getProds(nont)) {
                ...
                while (i < rnums.length) {
                    ...
                    let s = j;
                    while (s < i) addRangeToMapOfSet(retmap, rnums[s++], fset);
...
function mergeClosureSet(symnums: Iterable<number>, closuremap: Map<number, utility.closure.Closure>, map: Map<number, Set<number>>) {
    for (let symnum of symnums) {
        let closure = closuremap.get(symnum);
        if (closure == null) continue;
        for (let num of closure.getNodes()) {
            if (num === symnum) continue;
            let symset = map.get(num);
            if (symset == null) continue;
            for (let s of symset) addToMapOfSet(map, symnum, s);
        };
    };
}
