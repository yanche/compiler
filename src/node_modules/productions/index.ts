...
function createMultipleProductions(mprodstr: string): Array<prod.Production> {
...
export function createProdSet(mprodarr: Iterable<string>): prod.ProdSet {
    ...
    for (let mprod of mprodarr) {
        for (var p of createMultipleProductions(mprod)) prods.push(p);
    }
    return new prod.ProdSet(prods);
}

export {ProdSet, Symbol, Production as Production, ProductionRef} from './production';
