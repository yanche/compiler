...
export class IntermediateCode {
    ...
    toMIPS(classlookup: d.ClassLookup): m.MIPSAssembly {
    ...
    newCodePiece(fnsig: string, plen: number): CodePiece {
        let ret = new CodePiece(fnsig, plen, this._labelidgen);
...
export class RegInfo {
    ...
    mergeFrom(froms: Array<RegInfo>): boolean {
        if (this.type === RegInfo.TYPE_ANY) return false;
        let cons: number = null, regnum: number = null, type = this.type, rlen = froms.length;
        for (let i = 0; i < rlen; ++i) {
            let r = froms[i];
            if (r.type === RegInfo.TYPE_ANY) {
                this.type = RegInfo.TYPE_ANY;
                return true;
            }
            else if (type === RegInfo.TYPE_NEVER) {
                cons = r.cons;
                regnum = r.regnum;
                type = r.type;
            }
            else if (r.type === RegInfo.TYPE_CONST) {
                //here type must be CONST or CONST_TIMES_REG
                if (type !== RegInfo.TYPE_CONST || cons !== r.cons) {
                    this.type = RegInfo.TYPE_ANY;
                    return true;
                }
            }
            else if (r.type === RegInfo.TYPE_CONST_TIMES_REG) {
                if (type !== RegInfo.TYPE_CONST_TIMES_REG || regnum !== r.regnum || cons !== r.cons) {
                    this.type = RegInfo.TYPE_ANY;
                    return true;
                }
            }
...
export class CodeLabel {
    private _num: number;
    codeline: CodeLine;
    upstreams: Array<CodeLine>;
    //get num(): number { return this._num; }
    toString(): string {
        return 'label_' + this._num;
    }
    constructor(num: number) {
        this._num = num;
        this.upstreams = new Array<CodeLine>();
    }
}
...
export class CodePiece {
    ...
    addCodeLine(tac: t.TAC, label?: CodeLabel): this {
        if (this._stopgrow) throw new Error('cannot call because this CodePiece stops grow');
    ...
    newLabel(): CodeLabel {
        if (this._stopgrow) throw new Error('cannot call because this CodePiece stops grow');
        return new CodeLabel(this._labelidgen.next());
    }
    finalizeLabelRef(): this {
        this._stopgrow = true;
    ...
    initCodeTransferGraph(): this {
        if (!this._stopgrow) throw new Error('must be called after finalizeLabelRef');
        this._stopgrow = true;
        let codelines = this._codelines, regcount = this._tmpregid, clen = codelines.length;
        for (let i = 0; i < clen; ++i)
            codelines[i].initRegTrace(regcount);//.initRegLiveness();
        //initialize on parameter registers
        for (let r of codelines[0].regt_in.slice(0, this._plen))
            r.type = RegInfo.TYPE_ANY;
        return this;
    }
    valueInference(): this {
        ...
        while (stacktop > 0) {
            ...
            if (codeseq > 0) from = from.concat(this._codelines[codeseq - 1]);
            if (first || cl.makeInputRegTrace(from.map(n => n.regt_out))) {
                first = false;
                if (cl.bubbleOutputRegTrace()) {
                    let extrato = cl.branchToCL();
                    let tocodelines = extrato == null ? [] : [extrato];
                    if (codeseq !== this._codelines.length - 1)
                        tocodelines.push(this._codelines[codeseq + 1]);
    ...
    constructor(fnsig: string, plen: number, labelidgen: utility.IdGen) {
...
class CodeLine {
    regt_in: Array<RegInfo>;
    regt_out: Array<RegInfo>;
    ...
    //     return this;
    // }
    makeInputRegTrace(fromreginfos: Array<Array<RegInfo>>): boolean {
        let rlen = this.regt_in.length, changed = false;
        for (let i = 0; i < rlen; ++i) {
            let fromslice = fromreginfos.map(r => r[i]);
            let reginfo = this.regt_in[i];
            if (reginfo.mergeFrom(fromslice)) changed = true;
        }
        return changed;
    }
    bubbleOutputRegTrace(): boolean {
        let o = this.tac.regInfoOutput(this.regt_in), changed = false, rlen = this.regt_in.length;
        let newoutreg = this.regt_in.map(r => r.clone());
        if (o != null) newoutreg[o.regnum] = o.reginfo;
        for (let i = 0; i < rlen; ++i) {
            let rnew = newoutreg[i], rold = this.regt_out[i];
            if (rnew.type !== rold.type || rnew.cons !== rold.cons || rnew.regnum !== rold.regnum) changed = true;
            this.regt_out[i] = rnew;
        }
        return changed;
    }
    ...
    constructor(public tac: t.TAC, public linenum: number, public label?: CodeLabel) { }
}
