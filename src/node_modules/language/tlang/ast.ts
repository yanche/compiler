...
function completenesschecklist(list: Array<ASTNode>, noreturnallowed: boolean) {
    for (let l of list) {
        let cret = l.completenesscheck(noreturnallowed);
        if (!cret.accept) return cret;
    }
    return new util.SemanticCheckReturn(true);
}

function typecheckExprList(exprlist: Array<ASTNode_expr>, parr: Array<util.Type>, classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
    let len = exprlist.length;
    for (let i = 0; i < len; ++i) {
        let cret = exprlist[i].typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
...
export class ASTNode_functiondef extends ASTNode {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        return this.statementlist.typecheck(classlookup, fnlookup, ctx);
    }
    makeSymbolTable(classname: string, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        let symboltable = new c.SymbolFrame(null);
        if (classname) {
            //for method, constructor
            symboltable.add('this', new util.SymbolAttrs(new util.Type(classname, 0)));
        }
        for (let vardeclare of this.argumentlist.children) {
            let cret = vardeclare.makeSymbolTable(symboltable, classlookup);
            if (!cret.accept) return cret;
        }
        return this.statementlist.makeSymbolTable(symboltable, classlookup);
    }
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        let type = this.returntype.type;
        return this.statementlist.completenesscheck(type.depth === 0 && type.basetype === d.types.void);
    }
    genIntermediateCode(ic: i.IntermediateCode, context: i.IcContext) {
        let cp = ic.newCodePiece(this.fndef.signiture, this.fndef.argtypelist.length + (this.fndef.hasThisPtr ? 1 : 0));
        if (this.fndef.hasThisPtr) {
            let sattr = <util.SymbolAttrs>this._symbolframe.find('this');
            sattr.regId = cp.newTmpRegId();
        }
        //load parameters from stack
        this.argumentlist.genIntermediateCode(cp, context);
        this.statementlist.genIntermediateCode(cp, context);
        cp.addCodeLine(new tc.TAC_ret());
        cp.finalizeLabelRef();
    }
}
export class ASTNode_classdef extends ASTNode {
    ...
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        return this.body.completenesscheck(true);
    }
    genIntermediateCode(ic: i.IntermediateCode, context: i.IcContext) {
        ic.addGData(new i.GDATA_VTABLE(this._classdef));
        this.body.genIntermediateCode(ic, context);
    }
}
export class ASTNode_type extends ASTNode {
    ...
    // typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
    //     this._symbolframe = symbolframe;
    //     if (util.isType(this.type.basetype, classlookup)) return new util.SemanticCheckReturn(true);
    //     else return new util.SemanticCheckReturn(false, 'not a valid type at ' + this.area, 0);
    // }
}
export class ASTNode_argumentlist extends ASTNode {
    constructor(public children: Array<ASTNode_vardeclare>) { super(); }
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        let clen = this.children.length;
        for (let i = 0; i < clen; ++i) {
            let sattr = <util.SymbolAttrs>this._symbolframe.find(this.children[i].name.rawstr);
            sattr.regId = cp.newTmpRegId();
        }
    }
}
export class ASTNode_classbody extends ASTNode {
    ...
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        return completenesschecklist(this.children, true);
    }
    genIntermediateCode(ic: i.IntermediateCode, context: i.IcContext) {
        for (let c of this.children) {
            if (c instanceof ASTNode_functiondef)
                c.genIntermediateCode(ic, context);
            else if (c instanceof ASTNode_constructor)
                c.genIntermediateCode(ic, context);
        }
    }
}
...
export abstract class ASTNode_statement extends ASTNode {
    protected _symboltable: c.SymbolFrame;
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        return new util.SemanticCheckReturn(true);
    }
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
...
export class ASTNode_statementblock extends ASTNode_statement {
    ...
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        return this.statementlist.completenesscheck(noreturnallowed);
    }
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        this.statementlist.genIntermediateCode(cp, context);
    }
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        this._symboltable = symboltable;
        return this.statementlist.makeSymbolTable(symboltable.newFrame(), classlookup);
    }
}
export class ASTNode_statementlist extends ASTNode_statement {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        for (let l of this.children) {
            let cret = l.typecheck(classlookup, fnlookup, ctx);
            if (!cret.accept) return cret;
        }
        return new util.SemanticCheckReturn(true);
    }
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        ...
        for (; i < len; ++i) {
            let mret = this.children[i].completenesscheck(true);
            if (!mret.accept) return mret;
            if (mret.returns) break;
        }
        if (i === len) {
            if (noreturnallowed) return new util.SemanticCheckReturn(true);
            else return new util.SemanticCheckReturn(false, 'not all code path returns a value at ' + this.area, 0);
        }
        else if (i === len - 1) {
    ...
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        for (let c of this.children) c.genIntermediateCode(cp, context);
    }
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        this._symboltable = symboltable;
        for (let stmt of this.children) {
            let cret = stmt.makeSymbolTable(symboltable, classlookup);
            if (!cret.accept) return cret;
        }
        return new util.SemanticCheckReturn(true);
    }
...
export class ASTNode_vardeclare extends ASTNode_statement {
    ...
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        ...
        symboltable.add(this.name.rawstr, new util.SymbolAttrs(type));
        return new util.SemanticCheckReturn(true);
    }
}
export class ASTNode_if extends ASTNode_statement {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        let cretcond = this.cond.typecheck(classlookup, fnlookup, ctx);
        ...
        let cret = this.thenstat.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        return this.elsestat.typecheck(classlookup, fnlookup, ctx);
    }
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        let mret1 = this.thenstat.completenesscheck(true);
        if (!mret1.accept) return mret1;
        let mret2 = this.elsestat.completenesscheck(true);
    ...
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        let condreg = this.cond.genIntermediateCode(cp);
        let truelabel = cp.newLabel(), endlabel = cp.newLabel();
        cp.addCodeLine(new tc.TAC_btrue(truelabel, condreg));
        this.elsestat.genIntermediateCode(cp, context);
        cp.addCodeLine(new tc.TAC_branch(endlabel));
        cp.addCodeLine(new tc.TAC_noop(), truelabel);
        this.thenstat.genIntermediateCode(cp, context);
        cp.addCodeLine(new tc.TAC_noop(), endlabel);
    }
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        this._symboltable = symboltable;
        let cret = this.thenstat.makeSymbolTable(symboltable.newFrame(), classlookup);
        if (!cret.accept) return cret;
        return this.elsestat.makeSymbolTable(symboltable.newFrame(), classlookup);
    }
}
export class ASTNode_while extends ASTNode_statement {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        if (!cretcond.accept) return cretcond;
        if (!cretcond.type.isBool()) return new util.SemanticCheckReturn(false, 'expression of condition does not return bool at ' + this.cond.area, 0);
        return this.bodystmt.typecheck(classlookup, fnlookup, ctx);
    }
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        //even if this.bodystmt returns, while statement does not return
        let mret = this.bodystmt.completenesscheck(true);
        if (!mret.accept) return mret;
        return new util.SemanticCheckReturn(true);
    }
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        let condlabel = cp.newLabel(), endlabel = cp.newLabel();
        ...
        cp.addCodeLine(new tc.TAC_noop(), condlabel);
        let condreg = this.cond.genIntermediateCode(cp);
        cp.addCodeLine(new tc.TAC_bfalse(endlabel, condreg));
        this.bodystmt.genIntermediateCode(cp, context);
        cp.addCodeLine(new tc.TAC_branch(condlabel));
        cp.addCodeLine(new tc.TAC_noop(), endlabel);
    ...
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        this._symboltable = symboltable;
        return this.bodystmt.makeSymbolTable(symboltable.newFrame(), classlookup);
    }
}
export class ASTNode_dowhile extends ASTNode_while {
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
        //if this.bodystmt returns, then this do-while statement returns
        return this.bodystmt.completenesscheck(true);
    }
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        let startlabel = cp.newLabel(), endlabel = cp.newLabel();
        let old_lelabel = context.loopEndLabel, old_allabel = context.afterLoopLabel;
        context.loopEndLabel = startlabel;
        context.afterLoopLabel = endlabel;
        cp.addCodeLine(new tc.TAC_noop(), startlabel);
        this.bodystmt.genIntermediateCode(cp, context);
        let condreg = this.cond.genIntermediateCode(cp);
        cp.addCodeLine(new tc.TAC_btrue(startlabel, condreg));
        cp.addCodeLine(new tc.TAC_noop(), endlabel);
        //restore the old label context
        context.loopEndLabel = old_lelabel;
        context.afterLoopLabel = old_allabel;
...
export class ASTNode_for extends ASTNode_statement {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        let cret = this.initstmt.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        cret = this.cond.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        ...
        cret = this.endstmt.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        return this.bodystmt.typecheck(classlookup, fnlookup, ctx);
    }
    completenesscheck(noreturnallowed: boolean): util.SemanticCheckReturn {
    ...
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        let condlabel = cp.newLabel(), loopendlabel = cp.newLabel(), afterlooplabel = cp.newLabel();
        this.initstmt.genIntermediateCode(cp, context);
        let old_lelabel = context.loopEndLabel, old_allabel = context.afterLoopLabel;
        context.loopEndLabel = loopendlabel;
        context.afterLoopLabel = afterlooplabel;
        cp.addCodeLine(new tc.TAC_noop(), condlabel);
        let condreg = this.cond.genIntermediateCode(cp);
        cp.addCodeLine(new tc.TAC_bfalse(afterlooplabel, condreg));
        this.bodystmt.genIntermediateCode(cp, context);
        cp.addCodeLine(new tc.TAC_noop(), loopendlabel);
        this.endstmt.genIntermediateCode(cp, context);
        cp.addCodeLine(new tc.TAC_branch(condlabel));
        cp.addCodeLine(new tc.TAC_noop(), afterlooplabel);
    ...
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        ...
        this._symboltable = symboltable;
        let cret = this.initstmt.makeSymbolTable(symboltable, classlookup);
        if (!cret.accept) return cret;
        cret = this.endstmt.makeSymbolTable(symboltable, classlookup);
        if (!cret.accept) return cret;
        return this.bodystmt.makeSymbolTable(symboltable.newFrame(), classlookup);
    }
}
export class ASTNode_return extends ASTNode_statement {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        let cret = this.retexp.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
    ...
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        cp.addCodeLine(new tc.TAC_retreg(this.retexp.genIntermediateCode(cp)));
    }
}
export class ASTNode_break extends ASTNode_statement {
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        cp.addCodeLine(new tc.TAC_branch(context.afterLoopLabel));
    }
}
export class ASTNode_continue extends ASTNode_statement {
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        cp.addCodeLine(new tc.TAC_branch(context.loopEndLabel));
    }
}
...
export class ASTNode_vardefine extends ASTNode_statement {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        let cret = this.expr.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        let type = (<util.SymbolAttrs>this._symboltable.find(this.name.rawstr)).type;
    ...
    genIntermediateCode(cp: i.CodePiece, context: i.IcContext) {
        let exprreg = this.expr.genIntermediateCode(cp);
        let sattr = <util.SymbolAttrs>this._symbolframe.find(this.name.rawstr);
        sattr.regId = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_mov(exprreg, sattr.regId));
    }
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        this._symboltable = symboltable;
...
export abstract class ASTNode_expr extends ASTNode_statement {
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        throw new Error('not implemented');
    }
    makeSymbolTable(symboltable: c.SymbolFrame, classlookup: util.ClassLookup): util.SemanticCheckReturn {
        throw new Error('not implemented');
...
export class ASTNode_assignment extends ASTNode_expr {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        let cret = this.lval.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        let cretexpr = this.expr.typecheck(classlookup, fnlookup, ctx);
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        let exprreg = this.expr.genIntermediateCode(cp);
        let lvalret = this.lval.genIntermediateCode_lval(cp);
        if (lvalret.directReg) {
            cp.addCodeLine(new tc.TAC_mov(exprreg, lvalret.reg));
            return lvalret.reg;
        }
        else {
            let retreg = cp.newTmpRegId();
            if (lvalret.isbyte)
                cp.addCodeLine(new tc.TAC_sb(lvalret.reg, lvalret.offset, exprreg));
            else
                cp.addCodeLine(new tc.TAC_sw(lvalret.reg, lvalret.offset, exprreg));
            return retreg;
        }
    }
}
export class ASTNode_opexpr extends ASTNode_expr {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        let cret1 = this.expr1.typecheck(classlookup, fnlookup, ctx);
        if (!cret1.accept) return cret1;
        let cret2 = this.expr2.typecheck(classlookup, fnlookup, ctx);
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        let exprreg1 = this.expr1.genIntermediateCode(cp);
        let exprreg2 = this.expr2.genIntermediateCode(cp);
        let retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_binary(this.op.rawstr, exprreg1, exprreg2, retreg));
        return retreg;
    }
}
export class ASTNode_fncall extends ASTNode_expr {
    private _fndef: util.FunctionDefinition;
    constructor(public fn: ASTNode_fnref | ASTNode_baseconstructorref, public parameters: ASTNode_exprlist) { super(); }
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        let plen = this.parameters.children.length;
        let parr = new Array<util.Type>(plen);
        let cret = typecheckExprList(this.parameters.children, parr, classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        ...
        if (fn instanceof ASTNode_fnref) {
            ...
            if (ctx.baseclass.noConstructor && plen === 0) return new util.SemanticCheckReturn(true).setType(util.Type.void);
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        ...
        for (let p of this.parameters.children) {
            pregs.push(p.genIntermediateCode(cp));
        }
        for (let p of pregs.reverse()) {
            cp.addCodeLine(new tc.TAC_param(p));
        }
        if (this._constructor)
            cp.addCodeLine(new tc.TAC_param((<util.SymbolAttrs>this._symbolframe.find('this')).regId));
        let retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_fncall(this._fndef, pregs.length, retreg));
        return retreg;
    }
}
export class ASTNode_methodcall extends ASTNode_expr {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        let cret = typecheckExprList(this.parameters.children, parr, classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
        cret = this.obj.typecheck(classlookup, fnlookup, ctx);
        if (!cret.accept) return cret;
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        //value of 'this'
        let objreg = this.obj.genIntermediateCode(cp);
        let pregs = [objreg];
        for (let p of this.parameters.children) {
            pregs.push(p.genIntermediateCode(cp));
        }
        ...
        cp.addCodeLine(new tc.TAC_lw(objreg, 0, retreg));
        //now retreg has the runtime address of method
        cp.addCodeLine(new tc.TAC_binary_int('+', retreg, this._vtable_seq * 4, retreg));
        //load the method address into retreg
        cp.addCodeLine(new tc.TAC_lw(retreg, 0, retreg));
        //call the method
        cp.addCodeLine(new tc.TAC_fncall_reg(retreg, pregs.length, retreg));
        return retreg;
    }
}
class LVALICReturn {
...
export abstract class ASTNode_leftval extends ASTNode_expr {
    //for write into
    genIntermediateCode_lval(cp: i.CodePiece): LVALICReturn {
        throw new Error('not implemented');
    }
}
export class ASTNode_varref extends ASTNode_leftval {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        this._symbolframe = symbolframe;
        if (!symbolframe.has(this.token.rawstr)) return new util.SemanticCheckReturn(false, 'variable not found: ' + this.token.rawstr + ' at ' + this.token.area, 0);
        else return new util.SemanticCheckReturn(true).setType((<util.SymbolAttrs>symbolframe.find(this.token.rawstr)).type);
    }
    //for read
    genIntermediateCode(cp: i.CodePiece): number {
        return (<util.SymbolAttrs>this._symbolframe.find(this.token.rawstr)).regId;
    }
    //for write into
    genIntermediateCode_lval(cp: i.CodePiece): LVALICReturn {
        return new LVALICReturn(true, this.genIntermediateCode(cp), null, null);
    }
}
export class ASTNode_arrderef extends ASTNode_leftval {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        this._itemisbyte = util.typeEqualBool(rettype);
        return new util.SemanticCheckReturn(true).setType(rettype);
    }
    //for read
    genIntermediateCode(cp: i.CodePiece): number {
        let basereg = this.expr.genIntermediateCode(cp);
        let indexreg = this.indexexpr.genIntermediateCode(cp);
        let addrreg = cp.newTmpRegId(), retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_binary_int('*', indexreg, this._itemisbyte ? 1 : 4, addrreg));
        cp.addCodeLine(new tc.TAC_binary('+', basereg, addrreg, addrreg));
        cp.addCodeLine(new tc.TAC_lw(addrreg, 0, retreg));
        return retreg;
    }
    //for write into
    genIntermediateCode_lval(cp: i.CodePiece): LVALICReturn {
        let basereg = this.expr.genIntermediateCode(cp);
        let indexreg = this.indexexpr.genIntermediateCode(cp);
        let addrreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_binary_int('*', indexreg, this._itemisbyte ? 1 : 4, addrreg));
        cp.addCodeLine(new tc.TAC_binary('+', basereg, addrreg, addrreg));
        return new LVALICReturn(false, addrreg, 0, this._itemisbyte);
    }
}
export class ASTNode_fieldref extends ASTNode_leftval {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        this._fieldisbyte = util.typeEqualBool(field.type);
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        let basereg = this.expr.genIntermediateCode(cp);
        let addrreg = cp.newTmpRegId(), retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_binary_int('+', basereg, this._fieldoffset, addrreg));
        if (this._fieldisbyte)
            cp.addCodeLine(new tc.TAC_lb(addrreg, 0, retreg));
        else
            cp.addCodeLine(new tc.TAC_lw(addrreg, 0, retreg));
        return retreg;
    }
    ...
    genIntermediateCode_lval(cp: i.CodePiece): LVALICReturn {
        ...
        let addrreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_binary_int('+', basereg, this._fieldoffset, addrreg));
        return new LVALICReturn(false, addrreg, 0, this._fieldisbyte);
    }
}
...
export class ASTNode_literal_integer extends ASTNode_literal {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        this._symbolframe = symbolframe;
        return new util.SemanticCheckReturn(true).setType(new util.Type(d.types.int, 0));
    }
    genIntermediateCode(cp: i.CodePiece): number {
        let retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_loadint(Number(this.literal.rawstr), retreg));
        return retreg;
    }
}
export class ASTNode_literal_boolean extends ASTNode_literal {
    constructor(public literal: c.Token) { super(); }
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        return new util.SemanticCheckReturn(true).setType(new util.Type(d.types.bool, 0));
    }
    genIntermediateCode(cp: i.CodePiece): number {
        let retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_loadint(this.literal.rawstr === 'true' ? 1 : 0, retreg));
        return retreg;
    }
}
export class ASTNode_literal_null extends ASTNode_literal {
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        return new util.SemanticCheckReturn(true).setType(new util.Type(d.types.null, 0));
    }
    genIntermediateCode(cp: i.CodePiece): number {
        let retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_loadint(0, retreg));
        return retreg;
    }
}
export class ASTNode_newinstance extends ASTNode_expr {
    private _constructfndef: d.FunctionDefinition;
    private _noconstructor: boolean;
    private _vtablelabel: string;
    private _classbytes: number;
    constructor(public classname: c.Token, public parameters: ASTNode_exprlist) { super(); }
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        this._symbolframe = symbolframe;
        let plen = this.parameters.children.length;
        let parr = new Array<d.Type>(plen);
        for (let i = 0; i < plen; ++i) {
            let cret = this.parameters.children[i].typecheck(classlookup, fnlookup, ctx);
            if (!cret.accept) return cret;
            parr[i] = cret.type;
        }
        ...
        let type = new util.Type(this.classname.rawstr, 0);
        this._vtablelabel = classdef.getVTableLabel();
        if (classdef.noConstructor()) {
            if (plen === 0) {
                this._noconstructor = true;
                this._classbytes = classdef.byteLength;
                return new util.SemanticCheckReturn(true).setType(type);
            }
            ...
            let candidates = classdef.getApplicableConstructor(parr);
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        let objreg = cp.newTmpRegId(), tmpreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_loadint(this._classbytes, objreg));
        cp.addCodeLine(new tc.TAC_allocate(objreg, objreg));
        cp.addCodeLine(new tc.TAC_la(tmpreg, i.gdataLabel(this._vtablelabel)));
        cp.addCodeLine(new tc.TAC_sw(objreg, 0, tmpreg));
        if (!this._noconstructor) {
            let pregs = new Array<number>();
            for (let p of this.parameters.children) {
                pregs.push(p.genIntermediateCode(cp));
            }
            ...
            //value of 'this'
            cp.addCodeLine(new tc.TAC_param(objreg));
            cp.addCodeLine(new tc.TAC_procedurecall(this._constructfndef, pregs.length + 1));
        }
        return objreg;
    }
}
export class ASTNode_newarray extends ASTNode_expr {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        let cretexpr = this.exprlist.children[0].typecheck(classlookup, fnlookup, ctx);
        if (!cretexpr.accept) return cretexpr;
        if (!util.typeEqualInt(cretexpr.type)) return new util.SemanticCheckReturn(false, 'expression returns ' + cretexpr.type + ', not int at ' + this.exprlist.children[0].area);
        this._itembyte = util.typeEqualBool(new util.Type(this.type.basetype.rawstr, this.type.depth - 1));
        return new util.SemanticCheckReturn(true).setType(this.type.type);
    }
    genIntermediateCode(cp: i.CodePiece): number {
        let exprreg = this.exprlist.children[0].genIntermediateCode(cp);
        let objreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_binary_int('*', exprreg, this._itembyte ? 1 : 4, objreg));
        cp.addCodeLine(new tc.TAC_allocate(objreg, objreg));
        return objreg;
    }
}
export class ASTNode_unaryopexpr extends ASTNode_expr {
    ...
    typecheck(classlookup: util.ClassLookup, fnlookup: util.FunctionLookup, ctx: util.Context): util.SemanticCheckReturn {
        ...
        switch (this.token.rawstr) {
            case '~':
            case '-':
                if (!util.typeEqualInt(cretexpr.type)) return new util.SemanticCheckReturn(false, 'expression returns ' + cretexpr.type + ', not int at ' + this.expr.area);
                else return new util.SemanticCheckReturn(true).setType(cretexpr.type);
            case '!':
                if (!util.typeEqualBool(cretexpr.type)) return new util.SemanticCheckReturn(false, 'expression returns ' + cretexpr.type + ', not bool at ' + this.expr.area);
                else return new util.SemanticCheckReturn(true).setType(cretexpr.type);
            default:
    ...
    genIntermediateCode(cp: i.CodePiece): number {
        let exprreg = this.expr.genIntermediateCode(cp);
        let retreg = cp.newTmpRegId();
        cp.addCodeLine(new tc.TAC_unary(this.token.rawstr, exprreg, retreg));
        return retreg;
    }
}
