...
export function analysize(root: a.ASTNode_globaldefs): util.SemanticCheckReturn {
    let classlookup = new util.ClassLookup(), fnlookup = new util.FunctionLookup();

    //predefined class
    classlookup.addClass('Object', c.noArea);
    classlookup.getClass('Object').setParent(null);
    //predefined function
    fnlookup.addFn(util.predefinedFn.print_int.name, util.predefinedFn.print_int.rettype, util.predefinedFn.print_int.argtypelist, c.noArea);
    fnlookup.addFn(util.predefinedFn.print_bool.name, util.predefinedFn.print_bool.rettype, util.predefinedFn.print_bool.argtypelist, c.noArea);
    fnlookup.addFn(util.predefinedFn.print_newline.name, util.predefinedFn.print_newline.rettype, util.predefinedFn.print_newline.argtypelist, c.noArea);
    ...
    //return root.typecheck(new c.SymbolFrame(null), classlookup, fnlookup, new util.Context());
    ...
    for (let classname of classlookup.getAllClasses().concat(null)) {
        ...
        for (let fndef of fnlookup.findMethods(classname)) {
            ...
            if (!cret.accept) return cret;
            cret = fndef.astnode.typeAnalysis(classlookup, fnlookup, new util.Context(fndef.rettype, (classdef == null || fndef.name !== util.ClassLookup.constructorFnName) ? null : classdef.getParent(), classdef));
            if (!cret.accept) return cret;
...
function buildVTableAndFieldOnClass(classdef: util.ClassDefinition, fnlookup: util.FunctionLookup, colormap: Map<string, number>, classlookup: util.ClassLookup): util.SemanticCheckReturn {
    ...
    for (let fndef of selfdefmethods) {
        let i = 0;
        for (; i < plen; ++i) {
            if (selfvtable[i].signiture.equals2(fndef.signiture)) {
    ...
    let selffields = classdef.fieldSpace;
