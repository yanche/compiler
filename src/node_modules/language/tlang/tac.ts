...
export abstract class TAC {
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        throw new Error('not implemented');
    }
    toString(): string {
        throw new Error('not implemented');
    }
    inferTmpRegValue(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.TmpRegValueInference } {
        return null;
    }
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        return this;
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return [];
    }
    livenessProne(reg_live: Set<number>): TAC {
        return this;
    }
...
export class TAC_retreg extends TAC_ret {
    ...
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        let info = reginfos[this.reg];
        if (info.type === ic.RegInfo.TYPE_CONST) return new TAC_retint(info.cons);
        else if (info.type === ic.RegInfo.TYPE_CONST_TIMES_REG && info.cons === 1) return new TAC_retreg(info.regnum);
        else return this;
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
...
export class TAC_loadint extends TAC {
    ...
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        let retinfo = new ic.RegInfo();
        retinfo.cons = this.num;
        retinfo.type = ic.RegInfo.TYPE_CONST;
        return { regnum: this.to_reg, reginfo: retinfo };
    }
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        let info = reginfos[this.to_reg];
        if (info.type === ic.RegInfo.TYPE_CONST && info.cons === this.num) return new TAC_noop();
        else return this;
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return regLiveness_assign(reg_out, [], this.to_reg, true);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.to_reg) ? this : new TAC_noop();
    }
    writeReg(regnum: number): boolean {
...
export class TAC_mov extends TAC {
    ...
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        return { regnum: this.to_reg, reginfo: reginfos[this.from_reg].clone() };
    }
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        if (this.from_reg === this.to_reg) return new TAC_noop();
        let info1 = reginfos[this.from_reg], info2 = reginfos[this.to_reg];
        if (info1.type === ic.RegInfo.TYPE_CONST && info2.type === ic.RegInfo.TYPE_CONST && info1.cons === info2.cons) return new TAC_noop();
        else if (info1.type === ic.RegInfo.TYPE_CONST_TIMES_REG && info2.type === ic.RegInfo.TYPE_CONST_TIMES_REG && info1.cons === info2.cons && info1.regnum === info2.regnum) return new TAC_noop();
        else if (info1.type === ic.RegInfo.TYPE_CONST) return new TAC_loadint(info1.cons, this.to_reg);
        else if (info1.type === ic.RegInfo.TYPE_CONST_TIMES_REG) return new TAC_binary_int('*', info1.regnum, info1.cons, this.to_reg).simplify(reginfos);
        else return this;
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        if (this.from_reg === this.to_reg) return [];
        return regLiveness_assign(reg_out, [this.from_reg], this.to_reg, true);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.to_reg) ? this : new TAC_noop();
    }
...
export class TAC_allocate extends TAC {
    ...
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        let retinfo = new ic.RegInfo();
        retinfo.type = ic.RegInfo.TYPE_ANY;
        return { regnum: this.result_reg, reginfo: retinfo };
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return regLiveness_assign(reg_out, [this.reg_bytes], this.result_reg, true);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.result_reg) ? this : new TAC_noop();
    }
    readReg(regnum: number): boolean {
...
export class TAC_fncall extends TAC {
    constructor(public fn: d.FunctionDefinition, public plen: number, public result_reg: number) { super(); }
    toString(): string {
        return [reg2str(this.result_reg), '=', 'call', this.fn.signiture, this.plen].join(' ');
    }
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        let retinfo = new ic.RegInfo();
        retinfo.type = ic.RegInfo.TYPE_ANY;
    ...
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.result_reg) ? this : new TAC_procedurecall(this.fn, this.plen);
    }
    writeReg(regnum: number): boolean {
        return this.result_reg === regnum;
    }
    ...
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        return [
            ...
            new m.MIPS_jal(ic.fnLabel(this.fn.signiture)),
...
export class TAC_fncall_reg extends TAC {
    ...
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return regLiveness_assign(reg_out, [this.fn_reg], this.result_reg, false);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.result_reg) ? this : new TAC_procedurecall_reg(this.fn_reg, this.plen);
    }
    readReg(regnum: number): boolean {
...
export class TAC_procedurecall extends TAC {
    constructor(public fn: util.FunctionDefinition, public plen: number) { super(); }
    toString(): string {
        return ['call', this.fn.signiture, this.plen].join(' ');
    }
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        return [
            //call the function(return address into $ra)
...
export class TAC_binary extends TAC {
    ...
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        let retinfo = new ic.RegInfo(), oinfo1 = reginfos[this.operand1_reg], oinfo2 = reginfos[this.operand2_reg];

        if (oinfo1.type === ic.RegInfo.TYPE_NEVER || oinfo2.type === ic.RegInfo.TYPE_NEVER)
            throw new Error('defensive code, unclear behavior');
        else if (this.op === '*' && ((oinfo1.type === ic.RegInfo.TYPE_CONST && oinfo1.cons === 0) || (oinfo2.type === ic.RegInfo.TYPE_CONST && oinfo2.cons === 0))) {
            retinfo.type = ic.RegInfo.TYPE_CONST;
            retinfo.cons = 0;
            //TODO: MORE
        }
        else if (this.op !== '/' && oinfo1.type === ic.RegInfo.TYPE_CONST && oinfo2.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST;
            retinfo.cons = bi_op(this.op, oinfo1.cons, oinfo2.cons);
        }
        else if (this.op === '*' && oinfo1.type === ic.RegInfo.TYPE_CONST_TIMES_REG && oinfo2.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST_TIMES_REG;
            retinfo.cons = oinfo1.cons * oinfo2.cons;
            retinfo.regnum = oinfo1.regnum;
        }
        else if (this.op === '*' && oinfo2.type === ic.RegInfo.TYPE_CONST_TIMES_REG && oinfo1.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST_TIMES_REG;
            retinfo.cons = oinfo1.cons * oinfo2.cons;
            retinfo.regnum = oinfo2.regnum;
        }
        else if ((this.op === '+' || this.op === '-') && oinfo1.type === ic.RegInfo.TYPE_CONST_TIMES_REG && oinfo2.type === ic.RegInfo.TYPE_CONST_TIMES_REG && oinfo1.regnum === oinfo2.regnum) {
            if (this.op === '+')
                retinfo.cons = oinfo1.cons + oinfo2.cons;
            else
                retinfo.cons = oinfo1.cons - oinfo2.cons;
        ...
        else if (this.op === '*' && oinfo1.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST_TIMES_REG;
            retinfo.regnum = this.operand2_reg;
            retinfo.cons = oinfo1.cons;
        }
        else if (this.op === '*' && oinfo2.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST_TIMES_REG;
            retinfo.regnum = this.operand1_reg;
            retinfo.cons = oinfo2.cons;
        }
        else retinfo.type = ic.RegInfo.TYPE_ANY;

        // if (oinfo1.type === ic.RegInfo.TYPE_NEVER)
        //     retinfo = oinfo2.clone();
        // if (oinfo2.type === ic.RegInfo.TYPE_NEVER)
        //     retinfo = oinfo1.clone();
        return { regnum: this.result_reg, reginfo: retinfo };
    }
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        let oinfo1 = reginfos[this.operand1_reg], oinfo2 = reginfos[this.operand2_reg];
        if (this.op === '*' && ((oinfo1.type === ic.RegInfo.TYPE_CONST && oinfo1.cons === 0) || (oinfo2.type === ic.RegInfo.TYPE_CONST && oinfo2.cons === 0))) {
            return new TAC_loadint(0, this.result_reg);
            //TODO: MERGE WITH regInfoOutput
        }
        else if (this.op !== '/' && oinfo1.type === ic.RegInfo.TYPE_CONST && oinfo2.type === ic.RegInfo.TYPE_CONST) {
            return new TAC_loadint(bi_op(this.op, oinfo1.cons, oinfo2.cons), this.result_reg).simplify(reginfos);
        }
        else if (this.op === '*' && oinfo1.type === ic.RegInfo.TYPE_CONST_TIMES_REG && oinfo2.type === ic.RegInfo.TYPE_CONST) {
    ...
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return regLiveness_assign(reg_out, [this.operand1_reg, this.operand2_reg], this.result_reg, true);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.result_reg) ? this : new TAC_noop();
    }
...
export class TAC_binary_int extends TAC {
    ...
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        let retinfo = new ic.RegInfo(), oinfo = reginfos[this.operand_reg];

        if (oinfo.type === ic.RegInfo.TYPE_NEVER)
            throw new Error('defensive code, unclear behavior');
        else if (this.op === '*' && ((oinfo.type === ic.RegInfo.TYPE_CONST && oinfo.cons === 0) || this.operand_int === 0)) {
            retinfo.type = ic.RegInfo.TYPE_CONST;
            retinfo.cons = 0;
            //TODO: MORE
        }
        else if (this.op === '*' && oinfo.type === ic.RegInfo.TYPE_CONST_TIMES_REG) {
            retinfo.type = ic.RegInfo.TYPE_CONST_TIMES_REG;
            retinfo.cons = oinfo.cons * this.operand_int;
            retinfo.regnum = oinfo.regnum;
            //TODO: MORE
        }
        else if (oinfo.type === ic.RegInfo.TYPE_ANY)
            retinfo.type = ic.RegInfo.TYPE_ANY;
        else if (this.op !== '/' && oinfo.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST;
    ...
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        let oinfo = reginfos[this.operand_reg];
        if (this.op === '*' && ((oinfo.type === ic.RegInfo.TYPE_CONST && oinfo.cons === 0) || this.operand_int === 0)) {
            return new TAC_loadint(0, this.result_reg).simplify(reginfos);
            //TODO: MERGE WITH regInfoOutput
        }
        else if (this.op === '*' && oinfo.type === ic.RegInfo.TYPE_CONST_TIMES_REG) {
            let cons = oinfo.cons * this.operand_int;
            if (cons === 1) return new TAC_mov(oinfo.regnum, this.result_reg).simplify(reginfos);
            else return new TAC_binary_int('*', oinfo.regnum, cons, this.result_reg);
            //TODO: MORE
        }
        else if (this.op !== '/' && oinfo.type === ic.RegInfo.TYPE_CONST) {
            return new TAC_loadint(bi_op(this.op, oinfo.cons, this.operand_int), this.result_reg).simplify(reginfos);
        }
    ...
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return regLiveness_assign(reg_out, [this.operand_reg], this.result_reg, true);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.result_reg) ? this : new TAC_noop();
    }
...
export class TAC_unary extends TAC {
    ...
    regInfoOutput(reginfos: Array<ic.RegInfo>): { regnum: number, reginfo: ic.RegInfo } {
        ...
        if (oinfo.type === ic.RegInfo.TYPE_NEVER)
            ...
            retinfo.type = ic.RegInfo.TYPE_ANY;
        else if (oinfo.type === ic.RegInfo.TYPE_CONST) {
            retinfo.type = ic.RegInfo.TYPE_CONST;
            retinfo.cons = unary_op(this.op, oinfo.cons);
        }
        else if (oinfo.type === ic.RegInfo.TYPE_CONST_TIMES_REG && this.op === '-') {
            retinfo.type = ic.RegInfo.TYPE_CONST_TIMES_REG;
    ...
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        let oinfo = reginfos[this.operand_reg];
        if (oinfo.type === ic.RegInfo.TYPE_CONST) {
            return new TAC_loadint(unary_op(this.op, oinfo.cons), this.result_reg).simplify(reginfos);
        }
        else if (oinfo.type === ic.RegInfo.TYPE_CONST_TIMES_REG && this.op === '-') {
...
export class TAC_branch extends TAC {
    constructor(public label: ic.CodeLabel) { super(); }
    toString(): string {
...
export class TAC_btrue extends TAC_branch {
    constructor(label: ic.CodeLabel, public reg: number) { super(label); }
    toString(): string {
        return ['branch_true', reg2str(this.reg), this.label].join(' ');
    }
    simplify(reginfos: Array<ic.RegInfo>): TAC {
        let oinfo = reginfos[this.reg];
        if (oinfo.type === ic.RegInfo.TYPE_CONST) {
...
export class TAC_lw extends TAC {
    ...
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return regLiveness_assign(reg_out, [this.store_reg], this.result_reg, true);
    }
    livenessProne(reg_live: Set<number>): TAC {
        return reg_live.has(this.result_reg) ? this : new TAC_noop();
    }
...
export class TAC_la extends TAC {
    constructor(public to_reg: number, public label: util.Label) { super(); }
    toString(): string {
        return [reg2str(this.to_reg), '=', this.label].join(' ');
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
        return [{ regnum: this.to_reg, live: true }];
    }
    writeReg(regnum: number): boolean {
        return this.to_reg === regnum;
    }
    ...
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        return [
            new m.MIPS_la(r.toMIPSReg(this.to_reg, regmap), this.label.toString())
        ];
    }
}
...
//            false for a=fib(1,2)
function regLiveness_assign(reg_out: Set<number>, rhs_reg: Array<number>, lhs_reg: number, deadignore: boolean): Array<{ regnum: number, live: boolean }> {
    if (!reg_out.has(lhs_reg) && deadignore) return [];
