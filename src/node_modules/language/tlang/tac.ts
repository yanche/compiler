...
export class TAC_procedurecall extends TAC {
    constructor(public fn: d.FunctionDefinition, public plen: number) { super(); }
    toString(): string {
...
export class TAC_la extends TAC {
    constructor(public to_reg: number, public gdatalabel: string) { super(); }
    toString(): string {
        return [reg2str(this.to_reg), '=', this.gdatalabel].join(' ');
    }
    regLiveness(reg_out: Set<number>): Array<{ regnum: number, live: boolean }> {
    ...
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        return [
            new m.MIPS_la(r.toMIPSReg(this.to_reg, regmap), this.gdatalabel)
        ];
    }
}
