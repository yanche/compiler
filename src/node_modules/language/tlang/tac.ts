...
export class TAC_allocateint extends TAC {
    ...
    constructor(public num: number) { super(); }
    toString(): string {
        return ['allocateint', this.num].join(' ');
    }
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        //put the return integer into register v0
    }
...
export class TAC_allocate extends TAC {
    ...
    simplify(tmpreginfers: Array<i.TmpRegValueInference>): TAC {
        ...
        if (info.type === i.TmpRegValueInference.TYPE_CONST) return new TAC_allocateint(info.cons);
...
export class TAC_fncall extends TAC {
    constructor(public fn: util.FunctionDefinition, public plen: number, public result_reg: number) { super(); }
    toString(): string {
    ...
    livenessProne(tmpregbottomlive: Set<number>): TAC {
        return tmpregbottomlive.has(this.result_reg) ? this : new TAC_procedurecall(this.fn, this.plen);
    }
    writeReg(regnum: number): boolean {
        return this.result_reg === regnum;
    }
    ...
    toMIPSInstruction(regmap: Map<number, number>, retlabel: string, last: boolean, livetemps: Array<number>): Array<m.MIPSInstruction> {
        return [
            ...
            new m.MIPS_jal(i.fnLabel(this.fn.signiture)),
...
export class TAC_procedurecall_reg extends TAC {
    ...
    simplify(tmpreginfers: Array<i.TmpRegValueInference>): TAC {
        let info = tmpreginfers[this.fn_reg];
        if (info.type === i.TmpRegValueInference.TYPE_CONST_TIMES_REG && info.cons === 1) return new TAC_fncall_reg(info.regnum, this.plen);
        else return this;
    }
    tmpRegLiveness(tmpregbottomlive: Set<number>): Array<{ regnum: number, live: boolean }> {
