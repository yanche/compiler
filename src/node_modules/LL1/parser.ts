...
class LL1ParseTable {
    ...
    parse(tokens: Array<p.Token>, startsym: number): p.ParseReturn {
        if (!this._valid) return new p.ParseReturn(false, null, 'the grammar is not a valid LL(1)', 0)
        let stack = new Array<{ node: p.ParseTreeNode, symnum: number }>(), root = new p.ParseTreeMidNode(startsym, this._prodset), i = 0;
        stack.push({ node: root, symnum: startsym });
        while (stack.length > 0) {
            let stacktop = stack.pop(), token: p.Token = ((i === tokens.length) ? { rawstr: '', symnum: 0 } : tokens[i]);
            ...
            if (node instanceof p.ParseTreeTermNode) {
                ...
                if (prods.length === 0) return new p.ParseReturn(false, null, 'no production is found for: ' + node.symnum + ', ' + token.symnum, 2);
                if (prods.length > 1) return new p.ParseReturn(false, null, 'more than 1 productions are found for: ' + node.symnum + ', ' + token.symnum, 3);
                let newstackitems = this._prodset.getProdRef(prods[0]).rnums.map(sym => {
                    return { node: this._prodset.isSymNumTerminal(sym) ? new p.ParseTreeTermNode(sym, this._prodset) : new p.ParseTreeMidNode(sym, this._prodset), symnum: sym };
                });
...
export default class LL1Parser implements p.IParser {
