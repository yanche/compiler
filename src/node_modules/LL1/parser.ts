...
class LL1ParseTable {
    ...
    parse(tokens: Array<p.Token>, startsym: number): p.ParseReturn {
        let stack = new Array<{ node: p.ParseTreeNode, symnum: number }>(), root = new p.ParseTreeMidNode(this._prodset.getSymInStr(startsym)), i = 0;
        stack.push({ node: root, symnum: startsym });
        while (stack.length > 0) {
            let stacktop = stack.pop(), token: p.Token = ((i === tokens.length) ? { rawstr: '', symbol: '$' } : tokens[i]);
            ...
            if (node instanceof p.ParseTreeTermNode) {
                ...
                let newstackitems = this._prodset.getProdRef(prods[0]).rnums.map(sym => {
                    ...
                    if (this._prodset.isSymNumTerminal(sym)) return { node: new p.ParseTreeTermNode(symstr), symnum: sym };
                    else return { node: new p.ParseTreeMidNode(symstr), symnum: sym };
                });
        ...
        for (let nont of prodset.getNonTerminals()) {
            ...
            for (let prodid of prodset.getProds(nont)) {
                ...
                while (idx < rsymarr.length && gonull) {
                    ...
                    for (let f of firstsets.get(rsym)) syms.add(f);
                    ++idx;
                }
