"use strict";
class DFA {
    constructor(trans, start, terminals) {
        let statemap = new Map(), statect = 0;
        ...
        if (statect === 0)
            throw new Error('empty transitions is not acceptable');
        if (!statemap.has(start))
            throw new Error('start state number not found in transitions: ' + start);
        let terminalset = new Set();
        for (let ter of terminals) {
            if (!statemap.has(ter))
                throw new Error('terminal state number not found in transitions: ' + ter);
            terminalset.add(ter);
        }
        if (terminalset.size === 0)
            throw new Error('no terminal state is not acceptable');
        this._start = start;
    ...
    getStartState() { return this._start; }
    getTransitionMap(statenum) {
    ...
    equivalent(dfa) {
        if (this === dfa)
            return true;
        if (this._statect != dfa._statect || this._terminalset.size != dfa._terminalset.size)
            return false;
        let statemap = new Map(), stateset = new Set(), queue = [this._start];
        ...
        while (queue.length > 0) {
            ...
            if (trans1.size != state2.getTransitionMap().size)
                return false;
            for (let tran of trans1) {
                ...
                if (!map1has && !sethas) {
                    ...
                    if (statemap.get(tgtstate) != tgtstate2)
                        return false;
                }
                else
                    return false;
...
class State {
    ...
    addTransition(str, state) {
        if (this._tranmap.has(str))
            throw new Error('already has transition by given string: ' + str);
