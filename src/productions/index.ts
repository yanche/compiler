...
function createProduction(lstr: string, rstr: string): Production {
    ...
    if (startsWith(lstr, ProdSet.reservedNonTerminalPrefix)) throw new Error("non-terminal cannot start with preserved prefix: " + lstr);
    const lsymbol = new Symbol(false, lstr);
    const rsymbols = rstr.split(" ").filter(str => str.length > 0).map(str => {
        if (startsWith(str, ProdSet.reservedNonTerminalPrefix)) throw new Error("non-terminal cannot start with preserved prefix: " + str);
...
export { ProdSet, Production, ProductionRef } from "./production";
