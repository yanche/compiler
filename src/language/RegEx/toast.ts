...
import { ParseTreeMidNode, ParseTreeTermNode } from "../../compile";
import parser from "./parser";
import * as ap from "./astprocess";


let handlermap = new Map<string, (node: ParseTreeMidNode) => ap.ASTNode>();

//make the handlermap
function straightThruGen(pos: number): (node: ParseTreeMidNode) => ap.ASTNode {
    return function (node: ParseTreeMidNode): ap.ASTNode {
        return converter.toAST(<ParseTreeMidNode>node.children[pos]);
    };
}
...
//CONCAT & OR
function glueChildrenGen_OR(posright: number): (node: ParseTreeMidNode) => ap.ASTNode {
    return function (node: ParseTreeMidNode): ap.ASTNode {
...
function single(node: ParseTreeMidNode): ap.ASTNode {
    return new ap.ASTNode_Single((<ParseTreeTermNode>node.children[0]).token.rawstr.charCodeAt(0));
}
function range(node: ParseTreeMidNode): ap.ASTNode {
...
function childToAST(node: ParseTreeMidNode, pos: number): ap.ASTNode {
    return converter.toAST(<ParseTreeMidNode>node.children[pos]);
}

let converter = new ASTConverter<ASTNode>(parser.prodset, handlermap);
