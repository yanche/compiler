...
describe("token test", () => {
    it("keyword & special tokens", () => {
        ...
        const all = flatten([tripples, doubles, singles, keywords]);
        const lexRet = lex(all.join(" "), prodSet);
        assert.ok(lexRet.accept);
        assert.strictEqual(lexRet.tokens.length - 1, all.length);
        const lastToken = lexRet.tokens[lexRet.tokens.length - 1];
    ...
    it("primitive types, boolean", () => {
        const lexRet = lex("true false TRUE FALSE", prodSet);
        assert.ok(lexRet.accept);
    ...
    it("primitive types, integer", () => {
        const lexRet = lex("0 1 100", prodSet);
        assert.ok(lexRet.accept);
        const tokens = lexRet.tokens;
    ...
    it("primitive types, negative integer", () => {
        const lexRet = lex("-100 -1", prodSet);
        assert.ok(lexRet.accept);
        const tokens = lexRet.tokens;
        assert.strictEqual(tokens.length, 5);
    ...
    it("variables", () => {
        const lexRet = lex("abc a10 a_10 _abc_ ABC A__", prodSet);
        assert.ok(lexRet.accept);
        const tokens = lexRet.tokens;
