...
describe("value fold test", () => {
    it("cannot fold", () => {
        let num = 5;
        let codelines = genCodeLines([new t.TAC_retint(num)]);
    ...
    it("simple fold", () => {
        ...
        let r1 = regId.next();
        let codelines = genCodeLines([new t.TAC_loadint(num, r1), new t.TAC_retreg(r1)]);
    ...
    it("fold after return", () => {
        let num = 5, regId = new IdGen();
        let r1 = regId.next();
        let codelines = genCodeLines([new t.TAC_ret(), new t.TAC_loadint(num, r1), new t.TAC_retreg(r1)]);
        valueFold(codelines, inferValues(codelines, regId.cur, []));
