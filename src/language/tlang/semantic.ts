...
import { noArea } from "../../compile";
import { predefinedFn, ClassLookup, FunctionLookup, SemanticCheckReturn, isType, isFnRet, Type, FunctionDefinition, ClassDefinition, Field, SemContext } from "./util";
...
export default function analysize(root: ASTNode_globaldefs, classlookup: ClassLookup, fnlookup: FunctionLookup): SemanticCheckReturn {
    //predefined classes
    classlookup.addClass("Object", noArea);
    ...
    for (let node of root.children) {
        if (node instanceof ASTNode_classdef) {
            ...
            if (isType(nametoken.rawstr, classlookup)) return new SemanticCheckReturn(false, "cannot define class, name duplication: " + nametoken.rawstr + " at " + nametoken.area, 0);
    ...
    for (let node of root.children) {
        if (node instanceof ASTNode_classdef) {
            ...
            if (parentclass == null) return new SemanticCheckReturn(false, "not a valid class to inherit: " + parentclassname + " at " + (node.extendfrom == null ? noArea : node.extendfrom.area), 0);
            //set base class
            ...
            for (let b of node.body.children) {
                if (b instanceof ASTNode_vardeclare) {
                    ...
                    if (!isType(type.basetype, classlookup)) return new SemanticCheckReturn(false, "not a valid type: " + type.basetype + " at " + b.type.area, 0);
                    if (classdef.hasOwnField(b.name.rawstr)) return new SemanticCheckReturn(false, "duplicate field declaration: " + b.name.rawstr + " at " + b.name.area, 0);
                    classdef.addField(b.name.rawstr, type, b.area);
                }
                else {
                    //check type of argument list and return declaration
                    for (let a of b.argumentlist.children) { if (!isType(a.type.type.basetype, classlookup)) return new SemanticCheckReturn(false, "not a valid type: " + a.type.type + " at " + a.type.area, 0); }
                    if (!isFnRet(b.returntype.type.basetype, classlookup)) return new SemanticCheckReturn(false, "not a valid method return type: " + b.returntype.type + " at " + b.returntype.area, 0);
                    let argtypelist = [new Type(classdef.name, 0)].concat(toArgTypeList(b.argumentlist));
                    if (fnlookup.hasMethod(b.name.rawstr, classdef.name, argtypelist)) return new SemanticCheckReturn(false, "duplicate method/constructor declaration: " + b.name.rawstr + " at " + b.name.area, 0);
            ...
            //check type of argument list and return declaration
            for (let a of node.argumentlist.children) { if (!isType(a.type.type.basetype, classlookup)) return new SemanticCheckReturn(false, "not a valid type: " + a.type.type.basetype + " at " + a.type.area, 0); }
            if (!isFnRet(node.returntype.type.basetype, classlookup)) return new SemanticCheckReturn(false, "not a valid type: " + node.returntype.type.basetype + " at " + node.returntype.area, 0);
            //check existence of same function (by signiture)
            let argtypelist = toArgTypeList(node.argumentlist);
            if (fnlookup.hasFn(node.name.rawstr, argtypelist)) return new SemanticCheckReturn(false, "function already exists (with same signiture): " + node.name.rawstr + " at " + node.name.area, 0);
    ...
    if (mainfn.length === 0) return new SemanticCheckReturn(false, "entry function (main without parameter) is not found", 0);
    if (mainfn.length > 1) throw new Error("impossible code path, reserve for debugging purpose");
    if (!mainfn[0].rettype.isVoid()) return new SemanticCheckReturn(false, "entry function must return void, at " + mainfn[0].area, 0);
...
function buildVTableAndFieldOnClass(classdef: ClassDefinition, fnlookup: FunctionLookup, colormap: Map<string, number>, classlookup: ClassLookup): SemanticCheckReturn {
    if (colormap.get(classdef.name) === 1) return new SemanticCheckReturn(false, "class loop inheritance: " + classdef.name + " at " + classdef.area, 0);
    ...
    for (let fndef of selfdefmethods) {
        ...
        for (; i < plen; ++i) {
            if (selfvtable[i].overridedBy(fndef)) {
                ...
                else return new SemanticCheckReturn(false, "return type must be the same in method override: " + fndef.name + " at " + fndef.area, 0);
            }
        }
        if (i === plen) {
    ...
    for (let field of selffields) {
        for (let parentfield of parentfieldspace) {
            if (field.name === parentfield.name) return new SemanticCheckReturn(false, "cannot define field: " + field.name + " as its parent already has it, at " + parentfield.area, 0);
        }
    }
