...
function createGraph(arr: Iterable<utility.Edge>): DirectedGraph {
    var map = new Map<number, Set<number>>(), allnodes = new Set<number>();
    for (var edge of arr) {
        var srcnum = edge.src, tgtnum = edge.tgt;
        var edgeset = map.get(srcnum);
        if (edgeset == null) {
...
function calcClosure(graph: DirectedGraph): Map<number, Closure> {
    var colormap = new Map<number, number>(), closuremap = new Map<number, Closure>(), nlen = graph.nodes.size;
    if (nlen === 0) return closuremap;
    for (var i of graph.nodes) colormap.set(i, 0);
    var blackidx = 0, nodenums = [...graph.nodes];
    while (blackidx < nlen) {
...
function calcClosureOfNode(stack: Array<number>, stacktop: number, nodenum: number, colormap: Map<number, number>, edgesmap: Map<number, Set<number>>, closuremap: Map<number, Closure>) {
    ...
    var edgeset = edgesmap.get(nodenum);
    if (edgeset != null) {
        for (var adjnodenum of [...edgeset]) {
            if (adjnodenum === nodenum) continue; //self, ignore
            var adjcolor = colormap.get(adjnodenum), adjclosure = closuremap.get(adjnodenum);
            if (adjcolor === 2) //adj is black
                ...
                while (adjidx <= stacktop) {
                    var processedowners = new Set<number>();
                    for (var ownernodenum of closuremap.get(stack[adjidx++]).getOwnerNodes()) {
...
function addNodesToClosure(nodearr: Array<number>, s: number, e: number, closurenodenums: Iterable<number>, closuremap: Map<number, Closure>) {
    while (s <= e) {
        ...
        for (var i of closurenodenums) closure.addNode(i);
        ++s;
    }
}
...
function closureOfNodes(nodenums: Iterable<number>, closuremap: Map<number, Closure>): Set<number> {
    var ret = new Set<number>();
    for (var nodenum of nodenums) {
        for (var cnodenum of closuremap.get(nodenum).getNodes())
            ret.add(cnodenum);
    }
    return ret;
}
...
function calcClosureOfOneNode(edgemap: Map<number, Set<number>>, nodenum: number) {
    var set = new Set<number>().add(nodenum), queue = [nodenum];
    while (queue.length > 0) {
        var nnum = queue.pop();
        var adjset = edgemap.get(nnum);
        if (adjset == null) continue;
        for (var adjnum of adjset) {
            if (!set.has(adjnum)) {
                set.add(adjnum);
                queue.push(adjnum);
            }
